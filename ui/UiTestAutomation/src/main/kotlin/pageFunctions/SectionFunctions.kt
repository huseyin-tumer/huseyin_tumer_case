...
class SectionFunctions(driver: WebDriver) : BasePage(driver) {

    fun scrollToSection(sectionClass: String, waitForDomToStabilize: Boolean = false): WebElement {
        ...
        sections.forEach { section ->
            ...
            if (classValues.contains(sectionClass)) {
                scrollIntoView(section)
                waitForPageScrollingToStop()
                waitForAnimationToStop(section)
            }
            return@forEach
        }
        waitForPageScrollingToStop()
        val sectionElement = find(By.cssSelector("section.$sectionClass"))
        if (waitForDomToStabilize) waitForDomToStabilize()
        scrollIntoView(sectionElement)
        waitForPageScrollingToStop()
        waitForAnimationToStop(sectionElement)
        if (waitForDomToStabilize) waitForDomToStabilize()
        return sectionElement
    }

    fun scrollToDifferentiatorsTitle(element: WebElement, untilTextContains: String?) {
    ...
    fun assertDifferentiators(differentiatorsItems: Array<DifferentiatorsItem> = initialDifferentiators) {
        ...
        for (i in 0 until differentiatorsItems.size) {
            ...
            scrollToDifferentiatorsTitle(differentiatorItem, untilTextContains = differentiatorsItems[i].text)
            ...
            if (differentiatorsItems[i].text != null) {
                ...
                assertThat(TextUtil.textToSingleLine(text.text), equalTo(TextUtil.textToSingleLine(differentiatorsItems[i].text!!)))
            }
